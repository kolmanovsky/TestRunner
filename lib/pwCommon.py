...
class power():
    ...
    def sendCommand(self, command, port=None):
        ...
        if command not in self.commands.keys():
            log.error("Unknown command for power control.")
            return 'Error'

        log.debug("Issuing %s command to powerstrip", command)
        if port is None:
        ...
        try:
            ...
            log.error("Exception in pwCommons.")
            return 'Error'
        else:
            if issue.status_code == requests.codes.ok:
                if command != "status":
                    ...
                    for i, p in enumerate(reversed(str(issue.text.split(',')[1]))):
                        if p == "1":
                            log.debug("Port number %s is on" % str(i+1))
